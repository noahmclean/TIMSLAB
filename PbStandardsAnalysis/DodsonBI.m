function [ratiosBI, blockstats] = DodsonBI(dataRaw, MSmethod)
...
%   revised to include block means, Dec 13, 2017
...
m1 = ratioTimes - massTimes(massTable(:,2));
m2 = ratioTimes - massTimes(massTable(:,1));

%Use Dodson't simpler formulation -- easier for multivariate calculations
blocksBI = ( repmat(m1,1,size(firstCycle,2), size(firstCycle,3)).*firstCycle(massTable(:,1),:,:) + ...
             repmat(m2,1,size(secndCycle,2), size(secndCycle,3)).*secndCycle(massTable(:,1),:,:) ) ./ ...
           ( repmat(m2,1,size(firstCycle,2), size(firstCycle,3)).*firstCycle(massTable(:,2),:,:) + ...
             repmat(m1,1,size(secndCycle,2), size(secndCycle,3)).*secndCycle(massTable(:,2),:,:) );
         
%perform beam interpolation on partial blocks         
if partialBlockCycles
    
    partialBlockData = dataRaw(cyclesPerBlock*nBlocks+1:end,:)';
    partialBlockBI = (repmat(m1, 1, partialBlockCycles).* partialBlockData(massTable(:,1), 1:end-1) + ...
                      repmat(m2, 1, partialBlockCycles).* partialBlockData(massTable(:,1), 2:end) ) ./ ...
                     (repmat(m2, 1, partialBlockCycles).* partialBlockData(massTable(:,2), 1:end-1) + ...
                      repmat(m1, 1, partialBlockCycles).* partialBlockData(massTable(:,2), 2:end) ) ;
